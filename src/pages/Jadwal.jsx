  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
    <div className="bg-white rounded-lg w-full max-w-lg">
      ...
      <form onSubmit={(e) => {
        ...
        handleEditSubmit(e, selectedSchedule.studentPackageId, selectedSchedule.scheduleId);
      }}>
...
const Jadwal = () => {
  ...
  const transformScheduleData = (rawData, currentViewType) => {
    ...
    rawData.forEach((studentPackage) => {
      ...
      if (!studentPackage._id || !studentPackage.schedules || !Array.isArray(studentPackage.schedules)) {
        return;
      }
    ...
    console.log('Transformed Data with correct studentPackageId:', flattenedData);
  ...
  const handleEditAttendance = (studentPackageId, scheduleId) => {
    ...
    if (!studentPackageId) {
      console.error('Student Package ID not found', { studentPackageId, schedule });
  ...
  const handleEditSubmit = async (e, studentPackageId, scheduleId) => {
    ...
    if (!studentPackageId || !scheduleId) {
      console.error('Missing IDs in submission:', { studentPackageId, scheduleId });
    ...
    try {
      ...
      console.log('Sending request to:', `/api/student-packages/${studentPackageId}/schedules/${scheduleId}/attendance`);
      ...
      alert(error.response?.data?.message || 'Terjadi kesalahan saat mengupdate presensi');
    } finally {
  ...
  const fetchScheduleData = async () => {
    try {
      ...
      endpoint += isAdmin ? "all" : (getUserRole() === "student" ? "student" : "teacher");
      ...
      if (response.data.success) {
        const transformedData = transformScheduleData(response.data.data, viewType);
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
    <div className="bg-white rounded-lg w-full max-w-4xl">
      ...
      <div className="px-6 py-4 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <div className="mt-2 space-y-2">
              <p><span className="text-gray-600">Tanggal:</span> {attendanceData.date}</p>
              <p><span className="text-gray-600">Waktu:</span> {attendanceData.time}</p>
              <p><span className="text-gray-600">Ruangan:</span> {attendanceData.room}</p>
              <p><span className="text-gray-600">Pengajar:</span> {attendanceData?.teacher_id?.name}</p>
            </div>
          </div>
          <div>
        ...
        {/* Photo Section */}
        {attendanceData.activityPhoto && attendanceData.activityPhoto !== '-' && (
          <div>
            <h3 className="font-semibold text-gray-700 mb-2">Foto Aktivitas</h3>
...
const studentColumns = [
  ...
  },
  {
    ...
    width: "150",
    textAlign: "Right",
    template: (rowData) => <div>Rp {rowData.teacherFee.toLocaleString()}</div>,
  },
  {
    field: "note",
    headerText: "Note",
    ...
    <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl">
      <div className="flex justify-between items-center mb-6">
        <Header
          ...
          title={isAdmin ? "All Schedules" : `${getUserRole() === "student" ? "Student" : "Teacher"} Schedule`}
        />
        {loading && (
      ...
      <div className="flex justify-between items-center mb-4">
        <input
          ...
          placeholder={`Search ${isAdmin ? viewType : getUserRole()} schedule...`}
        />
